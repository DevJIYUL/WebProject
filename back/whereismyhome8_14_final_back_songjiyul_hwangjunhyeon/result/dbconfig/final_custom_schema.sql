...
CREATE TABLE IF NOT EXISTS `ssafyweb`.`favoritecate` (
  `username` VARCHAR(16) NOT NULL,
  `categoryCode` VARCHAR(3) NOT NULL,
  PRIMARY KEY (`username`, `categoryCode`),
  INDEX `fk_favorite_user_idx2` (`username` ASC) ,
  INDEX `fk_nsdpaknf_123_idx` (`categoryCode` ASC) ,
  CONSTRAINT `fk_favorite_user222`
    FOREIGN KEY (`username`)
    REFERENCES `ssafyweb`.`user` (`username`),
  CONSTRAINT `fk_nsdpaknf_123`
...
CREATE TABLE IF NOT EXISTS `ssafyweb`.`favoriteloc` (
  ...
  PRIMARY KEY (`username`, `dongCode`),
  INDEX `fk_favorite_users_idx` (`username` ASC) ,
  INDEX `fk_favorite_dongcode1_idx` (`dongCode` ASC) ,
  CONSTRAINT `fk_favorite_dongcode1`
...
CREATE TABLE IF NOT EXISTS `ssafyweb`.`fileinfo` (
  ...
  PRIMARY KEY (`idx`),
  INDEX `file_info_to_board_notice_no_fk222` (`noticeNo` ASC) ,
  CONSTRAINT `fk_file_info_to_board_notice_no_fk222`
...
CREATE TABLE IF NOT EXISTS `ssafyweb`.`quest` (
  ...
  `content` BLOB NULL DEFAULT NULL,
  `privateFlag` TINYINT NULL DEFAULT '0',
  `answeredFlag` TINYINT NULL DEFAULT '0',
...
-- -----------------------------------------------------
-- Table `ssafyweb`.`token`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafyweb`.`token` ;

CREATE TABLE IF NOT EXISTS `ssafyweb`.`token` (
  `username` VARCHAR(16) NOT NULL,
  `accessToken` VARCHAR(200) NULL DEFAULT NULL,
  `refreshToken` VARCHAR(200) NULL DEFAULT NULL,
  PRIMARY KEY (`username`),
  CONSTRAINT `fk_user_token_name`
    FOREIGN KEY (`username`)
    REFERENCES `ssafyweb`.`user` (`username`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4;
